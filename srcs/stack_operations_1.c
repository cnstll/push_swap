...
void	push(t_stack *dest_stack, t_stack *src_stack, char stack_name)
{
	if (!src_stack->is_empty)
	{
		...
		dest_stack->stack[dest_stack->top] = src_stack->stack[src_stack->top];
		src_stack->top++;
...
void	empty_stack(t_stack *dest_stack, t_stack *src_stack, char stk_name)
{
	while (check_if_empty(src_stack) != 1)
		push(dest_stack, src_stack, stk_name);
}
